...
class TestInstagramRedirectPreserved(unittest.TestCase):
    # go/kamakura/ is a live link from the client's Zenrise Instagram profile to
    # an OTA-tier tour. It exists ONLY in the production repo — staging has never
    # had a go/ directory, just as it has no relay/ — so asserting its presence
    # here would be wrong. The invariant that actually protects it is that this
    # build never writes to that path.
    def test_build_never_writes_to_the_instagram_path(self):
        written = bt.write_go_redirects([
            {'id': 'ikebana-ichigo-ichie', 'full': True,
             'widgets': {'en': f'{CH}/experience-calendar/1273232'}}])
        self.assertNotIn('go/kamakura/', written)
        self.assertEqual(written, ['go/ikebana-ichigo-ichie/'])
